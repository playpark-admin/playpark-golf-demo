...
function renderGame(){
  ...
  app.innerHTML=`<div class="game-shell immersive-game ${isPlus(state.membership)?'plus-game':''}"><header class="game-header"><button class="icon-button" data-action="pause" aria-label="일시 정지">${icon('back')}</button><div class="hole-heading"><span class="course-title"><span class="course-half">${r.holeIndex<9?'전반':'후반'}</span><b>${c.name}</b></span><h1><strong>${h.number}</strong>홀</h1><span class="hole-par">파 ${h.par}</span><span class="hole-length">${h.length}m</span></div></header>
  ...
  playerBadges=createPlayerBadges($('.field-area'),r,{plus:isPlus(state.membership),profileFor:p=>p.profile||savedProfile(p.name,p.id)});
...
function updateBalls(animatedId=null,position=null,trail=[]){
  ...
  playerBadges?.update({animatedId,position});
}
function applyZoom(){
