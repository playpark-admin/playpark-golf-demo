...
const state={page:'home',selected:[],mode:1,names:['나','친구 1','친구 2','친구 3'],round:saved,records:read('records',[]),learned:read('learned',[]),sound:read('sound',true),music:read('music',true),help:read('help',true),moving:false,angle:-Math.PI/2,power:40,view:'full',recordKey:null};
...
function syncMusic(){const r=state.round;gameMusic.setFocus(Boolean(r&&distance(r.players[r.active].ball,r.layout[r.holeIndex].cup)<=12));gameMusic.setActive(state.page==='game'&&Boolean(r)&&!document.hidden&&!modalRoot.querySelector('[data-music-pause]'));}
...
function shell(content){gameMusic.setActive(false);clearGameControls();app.innerHTML=`<header class="topbar app-header"><button class="app-brand" data-action="nav" data-page="home" aria-label="Park Player 홈"><img class="app-logo" src="./assets/brand/playpark-symbol.png" alt="플레이파크 로고" width="48" height="48"><strong>${APP_INFO.name}</strong></button></header><aside class="sidebar"><nav aria-label="주 메뉴">${navigation.map(([page,i,label])=>`<button class="nav-item ${state.page===page?'active':''}" data-action="nav" data-page="${page}" ${state.page===page?'aria-current="page"':''}>${icon(i)}<span>${label}</span></button>`).join('')}</nav><div class="side-caddy">${mascot()}<strong>저는 캐디 플팍!</strong><p>첫 샷부터 홀아웃까지<br>플팍이 함께해요.</p><button data-action="guide">게임 조작 알아보기 ${icon('arrow',15)}</button></div></aside><div class="workspace"><main id="main">${content}</main></div><nav class="mobile-nav" aria-label="모바일 메뉴">${navigation.map(([p,i,,t])=>`<button data-action="nav" data-page="${p}" class="${state.page===p?'active':''}" ${state.page===p?'aria-current="page"':''}>${icon(i,20)}<span>${t}</span></button>`).join('')}</nav>`;}
...
function coursePoints(name=state.coursePlayer){return playerGrowth(state.records,name).total;}
function normalizeCourseSelection(){const points=coursePoints();state.selected=state.selected.filter(id=>{const c=courses.find(c=>c.id===id);return c?.venueId===state.venueId&&courseAccess(c,points).unlocked;});}
...
function venueSection(select=true){const v=venueById(state.venueId);return venuePicker(v.id,coursePoints())+venueOverview(v.id,coursePoints())+'<div class="venue-courses-heading"><div><h2>2. '+v.name+'의 코스</h2><p>'+(select?'열린 코스 2개를 골라 전반·후반 9홀을 즐겨요.':'코스를 눌러 홀 구성과 해제 등급을 살펴보세요.')+'</p></div>'+(select?'<span class="selected-count"><b>'+state.selected.length+'</b> / 2 코스 선택</span>':'')+'</div><div class="course-grid '+(select?'':'atlas')+'">'+coursesAtVenue(v.id).map(c=>card(c,select)).join('')+'</div>';}
...
const rememberedPlayer=read('course-player',null);state.coursePlayer=typeof rememberedPlayer==='string'&&rememberedPlayer.trim()?rememberedPlayer.trim():saved?.players[0]?.name||state.records[0]?.players[0]?.name||'나';state.names[0]=state.coursePlayer;const knownCoursePlayers=read('course-players',[]);state.coursePlayers=[...new Set([state.coursePlayer,...(Array.isArray(knownCoursePlayers)?knownCoursePlayers.filter(n=>typeof n==='string'&&n.trim()):[])])];
