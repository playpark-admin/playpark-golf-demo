...
import {questions,byId,topics,levels,quizSource,normalizeProgress,recordAnswer,stats,filterQuestions,shuffledAnswers,pickQuestion,roundQuestion} from './quiz.js';
...
function shell(content){clearGameControls();app.innerHTML=`<aside class="sidebar"><button class="brand" data-action="nav" data-page="home"><img class="brand-lockup" src="./assets/brand/playpark-lockup.png" alt="PlayPark 플레이파크" width="130" height="137"></button><div class="side-label">PLAY, LEARN & CONNECT</div><nav aria-label="주 메뉴">${[['home','flag','플레이'],['courses','map','코스 도감'],['records','trophy','내 기록'],['rules','book','규칙 노트']].map(([page,i,label])=>`<button class="nav-item ${state.page===page?'active':''}" data-action="nav" data-page="${page}">${icon(i)}<span>${label}</span>${page==='home'?'<span class="new-dot"></span>':''}</button>`).join('')}</nav><div class="side-caddy">${mascot()}<strong>저는 캐디 플팍!</strong><p>첫 샷부터 홀아웃까지<br>플팍이 함께해요.</p><button data-action="guide">게임 조작 알아보기 ${icon('arrow',15)}</button></div><div class="side-footer"><span class="status-dot"></span> 오늘도, 즐거운 파크 라이프<small>© PLAYPARK · LITTLE GOLF, BIG FUN</small></div></aside><div class="workspace"><header class="topbar"><div class="breadcrumbs">플레이파크 <span>/</span> <strong>플팍 파크골프</strong><span class="beta">공개 BETA</span></div><div class="top-actions"><button class="icon-button" data-action="sound" aria-label="효과음 ${state.sound?'끄기':'켜기'}" aria-pressed="${state.sound}">${icon('sound',19)}${state.sound?'':'<i class="muted-line"></i>'}</button><button class="icon-button" data-action="guide" aria-label="게임 도움말">${icon('help',19)}</button><img class="avatar brand-avatar" src="./assets/brand/playpark-symbol.png" alt="플레이파크 로고" width="35" height="35"><span class="profile">파크 플레이어</span></div></header><main id="main">${content}</main></div><nav class="mobile-nav" aria-label="모바일 메뉴">${[['home','flag','플레이'],['courses','map','코스'],['records','trophy','기록'],['rules','book','규칙']].map(([p,i,t])=>`<button data-action="nav" data-page="${p}" class="${state.page===p?'active':''}">${icon(i,20)}<span>${t}</span></button>`).join('')}</nav>`;}
function render(){if(state.page==='game'){renderGame();return;}if(state.page==='home')home();if(state.page==='courses')coursePage();if(state.page==='records')recordsPage();if(state.page==='rules')rulesPage();}
...
async function shareScoreImage(){if(!cardExport)return;try{await navigator.share({files:[cardExport.file],title:'플레이파크 18홀 스코어카드'});}catch(e){if(e.name!=='AbortError')toast('공유할 수 없어요. 이미지 저장을 이용해 주세요.');}}
...
document.addEventListener('click',e=>{const btn=e.target.closest('[data-action]');if(!btn||btn.disabled)return;const a=btn.dataset.action;if(a==='close-event'){closeModal();return;}if(state.moving||state.eventOpen)return;switch(a){
  ...
  case 'nav':closeModal();state.page=btn.dataset.page;render();window.scrollTo(0,0);break;
...
window.PlayparkGame=Object.freeze({version:'0.6.0',ruleset:RULESET,getLastResult:()=>structuredClone(state.records[0]||null)});
