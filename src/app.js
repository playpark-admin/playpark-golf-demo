...
import {courses,RULESET} from './courses.js';
...
const state={page:'home',selected:['forest','lake'],mode:1,names:['나','친구 1','친구 2','친구 3'],round:saved,records:read('records',[]),learned:read('learned',[]),sound:read('sound',true),music:read('music',true),help:read('help',true),moving:false,angle:-Math.PI/2,power:40,view:'full',recordKey:null};
...
function normalizeCourseSelection(){const points=coursePoints();state.selected=state.selected.filter(id=>courseAccess(courses.find(c=>c.id===id),points).unlocked);}
...
document.addEventListener('click',e=>{const btn=e.target.closest('[data-action]');if(!btn||btn.disabled)return;const a=btn.dataset.action;if(a==='close-event'){closeModal();return;}if(state.moving||state.eventOpen)return;switch(a){
  ...
  case 'select-from-detail':if(!courseAccess(courses.find(c=>c.id===btn.dataset.id),coursePoints()).unlocked){toast('등급을 올리면 이 코스를 선택할 수 있어요.');break;}closeModal();state.page='home';if(!state.selected.includes(btn.dataset.id)){if(state.selected.length===2)state.selected.pop();state.selected.push(btn.dataset.id);}render();break;
