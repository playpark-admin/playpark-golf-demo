...
function shell(content){gameMusic.setActive(false);clearGameControls();app.innerHTML=`<div class="app-shell"><aside class="sidebar"><button class="app-brand" data-action="nav" data-page="home" aria-label="Park Player 홈"><img class="app-logo" src="./assets/brand/playpark-symbol.png" alt="플레이파크 로고" width="48" height="48"><strong>${APP_INFO.name}</strong></button><nav aria-label="주 메뉴">${navigation.map(([page,i,label])=>`<button class="nav-item ${state.page===page?'active':''}" data-action="nav" data-page="${page}" ${state.page===page?'aria-current="page"':''}>${icon(i)}<span>${label}</span></button>`).join('')}</nav><div class="side-caddy">${mascot()}<strong>저는 캐디 플팍!</strong><p>첫 샷부터 홀아웃까지<br>플팍이 함께해요.</p><button data-action="guide">게임 조작 알아보기 ${icon('arrow',15)}</button></div></aside><div class="workspace"><main id="main">${content}</main></div></div><nav class="mobile-nav" aria-label="모바일 메뉴">${navigation.map(([p,i,,t])=>`<button data-action="nav" data-page="${p}" class="${state.page===p?'active':''}" ${state.page===p?'aria-current="page"':''}>${icon(i,20)}<span>${t}</span></button>`).join('')}</nav>`;}
