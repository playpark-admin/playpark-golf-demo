...
function card(c,select=true){const selected=state.selected.indexOf(c.id);return `<article class="course-card ${select&&selected>=0?'selected':''}" style="--course-color:${c.color}"><button class="card-cover" data-action="${select?'select':'course-detail'}" data-id="${c.id}" aria-label="${c.name} ${select?(selected>=0?(selected===0?'전반':'후반')+' 9홀 선택됨, 선택 해제':'선택'):'상세 보기'}" ${select?`aria-pressed="${selected>=0}"`:''}>${c.preview==='map'?coursePreview(c):scene(c.id)}<span class="course-letter">${c.letter} COURSE</span>${select?`<span class="selection-check" aria-hidden="true">${selected>=0?icon('check',28):''}</span>`:''}<span class="course-tag">${c.tag}</span>${select&&selected>=0?`<span class="course-selection-banner"><b>선택됨</b><span>${selected===0?'전반':'후반'} 9홀</span></span>`:''}</button><div class="card-body"><div class="card-title"><button data-action="course-detail" data-id="${c.id}"><h3>${c.name}</h3></button><span class="difficulty" aria-label="난이도 ${c.level} / 3">${[1,2,3].map(n=>`<i class="${n<=c.level?'filled':''}"></i>`).join('')}</span></div><p>${c.description}</p><div class="card-meta"><span>9 HOLES <i>·</i> PAR 33</span>${select&&selected>=0?`<strong>${selected===0?'전반':'후반'} 9홀</strong>`:`<button data-action="course-detail" data-id="${c.id}" aria-label="${c.name} 둘러보기">${icon('arrow',17)}</button>`}</div></div></article>`;}
