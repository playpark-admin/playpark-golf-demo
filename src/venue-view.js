import {venueAccess} from './membership.js';
...
export function venuePicker(selectedId,points,context){
 return '<section class="venue-picker" aria-labelledby="venue-picker-title"><div class="venue-picker-heading"><h2 id="venue-picker-title">1. 구장 선택</h2><span>전국 테마 구장 '+venues.length+'곳</span></div><label class="field venue-mobile-picker">플레이할 구장<select id="venue-select">'+venues.map(v=>'<option value="'+v.id+'" '+(v.id===selectedId?'selected':'')+'>'+esc(v.region)+' · '+esc(v.name)+' ('+v.courseIds.length+'코스) · '+venueStatus(v,points,context)+'</option>').join('')+'</select></label><div class="venue-grid">'+venues.map(v=>{
  const count=coursesAtVenue(v.id).filter(c=>courseAccess(c,points,context).unlocked).length,selected=v.id===selectedId;
  return '<button class="venue-tile '+(selected?'is-selected':'')+'" data-action="venue" data-id="'+v.id+'" aria-pressed="'+selected+'">'+venueScenery(v)+'<span class="venue-check" aria-hidden="true">'+(selected?'✓':'')+'</span><small>'+esc(v.region)+'</small><b>'+esc(v.name)+'</b><span>'+v.courseIds.length+'코스 · '+count+'개 열림</span><span class="venue-membership-label">'+venueStatus(v,points,context)+'</span></button>';
 }).join('')+'</div></section>';
}
export function venueOverview(id,points,context){
 const v=venueById(id),all=coursesAtVenue(id),open=all.filter(c=>courseAccess(c,points,context).unlocked).length;
 return '<section class="venue-overview" data-venue-id="'+v.id+'" style="--venue-color:'+v.color+';--venue-light:'+v.light+'">'+venueScenery(v)+'<div><span class="venue-region">'+esc(v.region)+'</span><h2 tabindex="-1" id="active-venue-title">'+esc(v.name)+'</h2><p>'+esc(v.description)+'</p><ul class="venue-traits">'+v.features.map(f=>'<li>'+esc(f)+'</li>').join('')+'</ul><p class="venue-counts"><b>'+all.length+'개 코스 · '+all.length*9+'홀</b><span>'+open+'개 열림 · '+(all.length-open)+'개 잠김</span></p><p class="venue-membership-summary">'+venueStatus(v,points,context)+(context&&!venueAccess(v.id,points,context).unlocked?' · PLUS로 바로 이용하거나, 누적 포인트로 획득하세요.':'')+'</p></div></section>';
}
...
function venueStatus(v,points,context){if(!context)return '';const a=venueAccess(v.id,points,context);return !a.premium?'무료 기본 구장':a.owned?'획득 완료':a.unlocked?'PLUS 이용 가능':'PLUS 구장 · '+a.requiredPoints.toLocaleString('ko-KR')+' P에 획득';}
