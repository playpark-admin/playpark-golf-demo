import {distance,inBounds,segmentDistance,inwardNormal,boundaryCrossing} from './geometry.js';
export {distance,inBounds} from './geometry.js';
import {boundarySetting} from './course-features.js';
import {firstRockHit,clearOfObstacles} from './obstacles.js';
...
export function reliefPoint(origin,h){
  ...
  for(let r=.08;r<=1.7201;r+=.08)for(let i=0;i<=120;i++){
    ...
    if(inBounds(p,h)&&lieAt(p,h)!=='water'&&distance(p,h.cup)>=distance(origin,h.cup)-1e-8&&clearOfObstacles(p,h))return p;
  }
  return null;
}
...
export function simulateShot(h,start,angle,power){
  ...
  const impact=(kind,point)=>{impacts.push({kind,position:{...point},t:elapsed,speed:Math.hypot(vx,vy)});frames.push({...point,t:elapsed});event=kind;};
  const result=values=>({...values,impacts,crossings});
  for(let i=0;i<7200;i++){
    ...
    let inside=inBounds(p,h);
    if(wasInside!==inside){
      const crossing=boundaryCrossing(prev,p,h),setting=boundarySetting(crossing,h);
      if(setting.fence){
        const normal=inwardNormal(crossing,h),side=wasInside?1:-1,n={x:normal.x*side,y:normal.y*side},dot=vx*n.x+vy*n.y;
        impact('fence',crossing);vx=(vx-2*dot*n.x)*.48;vy=(vy-2*dot*n.y)*.48;
        p={x:crossing.x+n.x*.045,y:crossing.y+n.y*.045};if(inBounds(p,h)!==wasInside)p={...prev};inside=wasInside;
      }else{
        const mark={type:setting.type,direction:wasInside?'out':'in',position:{...crossing},t:elapsed};crossings.push(mark);
        if(wasInside)lastCross={...crossing};
        // Red sections prohibit crossing even if the ball subsequently returns.
        if(setting.type==='red'&&!redCross)redCross={...crossing};
        if(!wasInside&&setting.type==='white'&&!redCross)event='return';
      }
    }
    wasInside=inside;
    // Once a red boundary has been crossed, neither re-entry nor cup-in cancels OB.
    const cupOffset=segmentDistance(h.cup,prev,p),approaching=(h.cup.x-prev.x)*(p.x-prev.x)+(h.cup.y-prev.y)*(p.y-prev.y)>0;
    if(!redCross&&cupOffset<CUP_RADIUS&&approaching){const entrySpeed=Math.hypot(vx,vy),captureSpeed=CUP_MAX_ENTRY_SPEED*(1-.35*(cupOffset/CUP_RADIUS)**2);if(entrySpeed<=captureSpeed){p={...h.cup};frames.push({...p,t:elapsed});return result({position:p,frames,holed:true,penalty:0,event:'cup'});}event='cup-miss';}
    if(inside&&h.water.some(e=>ellipse(p,e))){frames.push({...p,t:elapsed});if(redCross)break;return result({position:{...p},frames,holed:false,penalty:0,event:'water',needsRelief:true});}
    if(i%4===0)frames.push({...p,t:elapsed});
  }
  frames.push({...p,t:elapsed});
  if(redCross||!inBounds(p,h)){const origin=redCross||lastCross,legal=obReliefPoint(origin,h);return result({position:legal,frames,holed:false,penalty:2,event:'ob',obReason:redCross?'red-crossing':'outside',reliefOrigin:origin,usedObTee:false});}
  return result({position:p,frames,holed:false,penalty:0,event:event||lieAt(p,h)});
}
...
export function takeRelief(r){if(r.status!=='playing')throw Error('처치할 수 없는 상태입니다.');const p=r.players[r.active],h=r.layout[r.holeIndex];if(!p.strokes)throw Error('티샷 전에는 처치하지 않습니다.');const from={...p.ball};let relief=reliefPoint(p.ball,h);
  if(!relief){
    ...
    for(let d=.2;d<Math.max(h.width,h.height)*2;d+=.2){const candidate={x:p.ball.x+Math.cos(a)*d,y:p.ball.y+Math.sin(a)*d};if(inBounds(candidate,h)&&lieAt(candidate,h)!=='water'&&clearOfObstacles(candidate,h)){relief=candidate;break;}}
  }
