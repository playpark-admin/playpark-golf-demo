...
export function roundGrowthView(record,records,{final=false,stored=true,membership}={}){
 ...
 const a=roundGrowth(record,name),g=playerGrowth(records,name),before=playerGrowth(records.filter(r=>r.id!==record.id),name),opened=final&&stored?newlyUnlockedCourses(before.total,g.total,membership):[];
