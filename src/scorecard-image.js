...
export async function scorecardImage(record,{theme='classic'}={}){
 ...
 if(gold){const g=ctx.createLinearGradient(0,0,w,206);g.addColorStop(0,'#d6b36b00');g.addColorStop(.7,'#d6b36b20');g.addColorStop(1,'#d6b36b08');box(1,1,w-2,199,g);ctx.strokeStyle='#d6b36b55';ctx.lineWidth=1;for(let x=500;x<1300;x+=60){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x-160,202);ctx.stroke();}try{const crest=await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=new URL('../assets/brand/plus-crest.svg',import.meta.url).href;});ctx.save();ctx.globalAlpha=.16;ctx.drawImage(crest,920,28,174,174);ctx.restore();}catch{}ctx.strokeStyle='#c5a253';ctx.lineWidth=2;ctx.strokeRect(16,220,w-32,h-238);}
